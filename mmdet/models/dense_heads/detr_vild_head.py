...
class DETRViLDHead(DETRHead):
    """DETR transformer head with ViLD loss.
    ...
    def __init__(self,
                 ...
                 loss_img=dict(type='L1Loss', loss_weight=2.0),
                 train_cfg=dict(
                     assigner=dict(
        ...
        if class_weight is not None and (self.__class__ is DETRViLDHead):
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict,
        ...
        if (version is None or version < 2) and self.__class__ is DETRViLDHead:
            convert_dict = {
    ...
    def forward(self, feats, img_metas):
        ...
        cls_emb = torch.cat((self.text_embeddings, self.bg_embedding), dim=0).t()

        logit_scale_list = [logit_scale for _ in range(num_levels)]
        cls_emb_list = [cls_emb for _ in range(num_levels)]
        return multi_apply(self.forward_single, feats, img_metas_list,
                           logit_scale_list, cls_emb_list)

    def forward_single(self, x, img_metas, logit_scale, cls_emb):
        ...
        all_region_embeds = F.normalize(self.fc_cls(outs_dec), dim=-1)
        all_cls_scores = logit_scale * all_region_embeds @ cls_emb
        all_bbox_preds = self.fc_reg(self.activate(
            self.reg_ffn(outs_dec))).sigmoid()
        return all_cls_scores, all_bbox_preds, all_region_embeds
    ...
    def loss(self,
        ...
        all_region_embeds = all_region_embeds[-1]
        ...
        all_gt_embeds_list = [gt_embeds_list for _ in range(num_dec_layers)]
        all_gt_embed_weights_list = [
            gt_embed_weights for _ in range(num_dec_layers)]
        all_gt_bboxes_ignore_list = [
            gt_bboxes_ignore for _ in range(num_dec_layers)
        ]
        img_metas_list = [img_metas for _ in range(num_dec_layers)]

        losses_cls, losses_bbox, losses_iou, losses_img = multi_apply(
            self.loss_single, all_cls_scores, all_bbox_preds,
            all_region_embeds, all_gt_bboxes_list, all_gt_labels_list, 
            all_gt_embeds_list, all_gt_embed_weights_list, img_metas_list,
            all_gt_bboxes_ignore_list)
        ...
        loss_dict['loss_img'] = losses_img[-1]
        # loss from other decoder layers
        num_dec_layer = 0
        for loss_cls_i, loss_bbox_i, loss_iou_i, loss_img_i \
            in zip(losses_cls[:-1], losses_bbox[:-1],
                   losses_iou[:-1], losses_img[:-1]):
            loss_dict[f'd{num_dec_layer}.loss_cls'] = loss_cls_i
            loss_dict[f'd{num_dec_layer}.loss_bbox'] = loss_bbox_i
            loss_dict[f'd{num_dec_layer}.loss_iou'] = loss_iou_i
            loss_dict[f'd{num_dec_layer}.loss_img'] = loss_img_i
            num_dec_layer += 1
        return loss_dict
    ...
    def loss_single(self,
        ...
        cls_reg_targets = self.get_targets(cls_scores_list, bbox_preds_list,
                                           ...
                                           gt_embeds_list, gt_embed_weights_list,
                                           img_metas, gt_bboxes_ignore_list)
        (labels_list, label_weights_list, bbox_targets_list, bbox_weights_list,
         embeds_list, embed_weights_list, num_total_pos, num_total_neg) = \
         cls_reg_targets
        ...
        embed_weights = torch.cat(embed_weights_list, 0)
        ...
        region_embeds = region_embeds.reshape(-1, self.word_embed_dim)
        loss_img = self.loss_img(
            region_embeds, embeds, embed_weights, avg_factor=num_total_pos)
    ...
    def get_targets(self,
        ...
        (labels_list, label_weights_list, bbox_targets_list,
         bbox_weights_list, embeds_list, embed_weights_list,
         pos_inds_list, neg_inds_list) = multi_apply(
             self._get_target_single, cls_scores_list, bbox_preds_list,
             gt_bboxes_list, gt_labels_list, gt_embeds_list,
             gt_embed_weights_list, img_metas, gt_bboxes_ignore_list)
    ...
    def _get_target_single(self,
        ...
        return (labels, label_weights, bbox_targets, bbox_weights, 
                embeds, embed_weights, pos_inds, neg_inds)

    # over-write because gt_embeds, gt_embed_weights are needed as inputs
    # for image loss.
    def forward_train(self,
        ...
        Args:
            ...
            gt_embeds (list[Tensor]): Image embeddings corresponding to each box,
                shape (num_gts, word_embed_dim)
            gt_embed_weights (list[Tensor]): Weight of image embeddings for weighted
                embedding loss, shape (num_gts, word_embed_dim)
        ...
        loss_inputs = outs + (gt_bboxes, gt_labels, gt_embeds,
    ...
    def forward_single_onnx(self, x, img_metas, logit_scale, cls_emb):
