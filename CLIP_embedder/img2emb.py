...
for i in tqdm(img_ids):
    ...
    for ann in ann_info:
        ...
        pred_label = int(torch.argmax((100.0 * im_embed @ text_embed.T).softmax(dim=-1), dim=-1))
        is_correct = BASE_CLASSES[pred_label] == coco.cats[ann['category_id']]['name']
