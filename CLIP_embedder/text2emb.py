...
with torch.no_grad():
    base_features = model.encode_text(base_inputs)
    novel_features = model.encode_text(novel_inputs)
...
print("Base features:", base_features.shape)
print("Novel features:", novel_features.shape)

with open(osp.join(save_dir, base_fname), 'wb') as out:
    pickle.dump(base_features, out, pickle.HIGHEST_PROTOCOL)
with open(osp.join(save_dir, novel_fname), 'wb') as out:
    pickle.dump(novel_features, out, pickle.HIGHEST_PROTOCOL)